...
###### 156. Why does the setTimeout callback even with 0ms delay always execute after the synchronous code in the global scope?
...
- A: `'Hello from callback' after 2 seconds, then 'Hellow from global scope'`
- B: `If setTimeout is 0ms, 'Hello from callback' runs first`
- C: `The callback executes as soon as the timer ends, regardless of the main thread`
- D: `Hello from global scope & Hello from callback(after 2s)`
...
Synchronous code(main thread) runs immediately. The `setTimeout` is queued after the daelay and executes only when the call stack is empty, even with `0ms`.

1. `setTimeout Scheduling:` When `setTimout` is called, the Web API starts a timer(2 seconds here). Once the timer elapses, the callback is placed in the task queue(or "callnack queue").

2. `Main THread Excution:` The `console.log('Hello from global scope')` runs immediately because it's on the main thread isn"t blocked by `setTimeout`, so this line executes right after scheduling the timer.

3. `Event Loop Behavior:` The event loop continuously checks if the call stack is empty. only when the main thread(synchronous code) finishes the event loop push the queued callback from the task queue to the call stack for execution.

4. `Zero-Delay Case:` Even with `setTimeout(fn, 0)`, the callback is queued, not executed immediately. The main thread's `console.log` still runs first, demonstrating that asychronous code(even with 0ms delay) always yields to the current synchronous execution.
